...
const DATA_DIR  = process.env.DATA_DIR || path.join(__dirname, 'transcripts');

// Crear carpeta de transcripts si no existe
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

// Validar hash: solo hex de 40 chars (crypto.randomBytes(20).toString('hex'))
...
// Leer body completo de la request
...
const server = http.createServer(async (req, res) => {
  ...
  if (method === 'GET' && url === '/') {
    ...
    return res.end(JSON.stringify({ status: 'ok', service: 'VortexGG Transcript Server' }));
  }
  ...
  if (method === 'POST' && url === '/upload') {
    try {
      ...
      const { hash, html } = JSON.parse(body);
      ...
      if (typeof html !== 'string' || html.length < 10) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ error: 'HTML inválido' }));
      }

      fs.writeFileSync(filePath(hash), html, 'utf8');
      console.log(`[Upload] Guardado transcript ${hash}`);
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   Transcripts guardados en: ${DATA_DIR}`);
});
